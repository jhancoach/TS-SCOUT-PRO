...
export const TeamCard: React.FC<TeamCardProps> = ({ team, onClick }) => {
  return (
    <div 
      ...
      className="group relative overflow-hidden rounded-xl bg-zinc-900 border border-zinc-800 hover:border-yellow-500/50 transition-all duration-300 cursor-pointer hover:shadow-2xl hover:shadow-yellow-900/10 hover:-translate-y-1 h-full flex flex-col"
    >
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-4">
          ...
          <span 
            className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-zinc-950 border border-zinc-800 text-zinc-400"
          >
            OPÇÃO {team.id.replace('team', '')}
          </span>
        </div>

        <p className="text-zinc-400 mb-8 text-sm h-10 line-clamp-2 border-l-2 border-zinc-800 pl-3">
          {team.summary}
        </p>

        {/* Quick Stats Grid */}
        <div className="grid grid-cols-2 gap-3 mb-8">
          <div className="bg-zinc-950 p-3 border border-zinc-800/50 rounded flex items-center gap-3">
        ...
        <div className="mt-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {team.roster.map((player, idx) => (
              <span key={idx} className="text-xs bg-zinc-800 text-zinc-300 border border-zinc-700 px-2 py-1 rounded-sm">
                {player.name}
              </span>
            ))}
          </div>
      ...
      {/* Hover Effect Footer */}
      <div className="bg-zinc-950 p-4 flex justify-between items-center text-xs text-zinc-500 group-hover:text-yellow-500 transition-colors border-t border-zinc-800">
